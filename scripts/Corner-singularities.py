...
def singularity(r, theta, phi=np.pi / 2, eta=1.0):
    ...
    mask = theta >= phi / 2
    u[mask] *= np.cosh(eta * (np.pi - theta[mask])) / np.cosh(
        eta * (np.pi - phi / 2)
    )
    mask = theta <= -phi / 2
    u[mask] *= np.cosh(eta * (np.pi + theta[mask])) / np.cosh(
...
u_an_Euler = lambda z, theta, eta: singularity(
    R * np.exp(z), theta, phi=phi, eta=eta
)
...
circ = mpl.patches.Wedge(
    (0, 0), R, theta1=0, theta2=360, transform=ax.transData
)
...
pa = mpl.lines.Line2D(
    [R * np.cos(-phi / 2), 0, R * np.cos(phi / 2)],
...
ax.text(0.9 * R, 0, r"$\Omega$", horizontalalignment="right")
ax.text(0, 0.9 * R, r"$\Omega^c$", verticalalignment="top")
# Normal vector
nv_rpos, nv_length = 0.8 * R, 0.25 * R
pa = mpl.patches.Arrow(
    nv_rpos * np.cos(phi / 2),
    nv_rpos * np.sin(phi / 2),
...
ax.add_patch(pa)
ax.text(
    nv_rpos * np.cos(phi / 2) + nv_length * (-np.sin(phi / 2)),
    nv_rpos * np.sin(phi / 2) + nv_length * (np.cos(phi / 2)),
    r"$\nu$",
    horizontalalignment="right",
)
# Corner region {r=R}
pa = mpl.patches.Circle(
    (0, 0),
    R,
    edgecolor="k",
    facecolor="none",
...
ax.add_patch(pa)
ax.text(
    -R / np.sqrt(2), R / np.sqrt(2), r"$r=R $", horizontalalignment="center"
)
...
pb_rpos = 0.7 * nv_rpos
pa = mpl.patches.Arrow(
    pb_rpos * np.cos(pb_angle),
    pb_rpos * np.sin(pb_angle),
...
ax.add_patch(pa)
ax.text(
    pb_rpos * np.cos(pb_angle) + nv_length * (np.cos(pb_angle)),
...
ax.add_patch(pa)
ax.text(
    pb_rpos * np.cos(pb_angle) + nv_length * (-np.sin(pb_angle)),
...
z_, theta_ = np.meshgrid(
    np.linspace(z_min, z_max, num=int(nz)),
...
ax.text(0.9 * z_alpha, 0, r"$\Psi(\Omega)$", horizontalalignment="left")
ax.annotate(
    r"$\Psi(\Gamma)$",
...
pa = mpl.patches.Arrow(nv_zpos, phi / 2, 0, nv_length, width=0.1, facecolor="k")
ax.add_patch(pa)
ax.text(nv_zpos, phi / 2 + nv_length, r"$\nu$", verticalalignment="bottom")
# Corner region
pa = mpl.lines.Line2D([0, 0], [-np.pi, np.pi], color="k", linestyle="dotted")
ax.add_artist(pa)
# Scaling region
pa = mpl.lines.Line2D(
    [np.log(R_alpha / R), np.log(R_alpha / R)],
    [-np.pi, np.pi],
    color="k",
    linestyle="--",
)
...
fig, ax = plt.subplots(
    ncols=2, nrows=1, sharey=True, layout="constrained", figsize=figsize
)
...
Z = np.real(
    singularity_scaled(r, phi / 2, eta=eta, alpha=alpha, R_alpha=R_alpha)
)
Z = Z / np.max(np.abs(Z))
ax[0].plot(
    r,
    Z,
    label=r"$\alpha=\exp(i\pi/val)$".replace(
        "val", f"{np.pi / np.angle(alpha):2.2g}"
    ),
)
