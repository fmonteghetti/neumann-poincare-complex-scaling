...
for element_size in element_size_l:
    (eigval, dof) = Num_BEM.compute_NP_eigenvalues_ellipse_isogeometric(
        a,
        b,
        opts_eigv,
        meshsize=element_size,
        qorder=qorder,
        qmaxdist=4 * element_size,
    )
...
for i, N_error in enumerate(N_error_l):
    error = error_l[i, :]
    p = Num_utils.compute_slope(
        np.array(dof_l), np.array(error), N=len(element_size_l)
    )
    ax.loglog(
...
ax.set_title(
    f"BIE error, $P$={int(np.ceil((qorder + 1) / 2))}, ($a$,$b$)=({a},{b})"
)
ax.legend()
ax.grid(which="both")
