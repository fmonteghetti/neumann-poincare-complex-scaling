...
for alpha in alpha_l:
    (eigval, dof, _, _) = compute_eigenvalues_BEM(
        gmshfile, opts_eigv, qmaxdist=10 * element_size, pml_param=alpha
    )
...
f.suptitle(
    f"Eigenvalues, major axis perturbation: $(a,b,\phi)=$"
...
gmshfile = Num_utils.build_mesh(geofile, geo_param, gmsh_param, gdim=1)
(eigval_bem, dof_bem, _, _) = compute_eigenvalues_BEM(
    gmshfile, opts_eigv, qmaxdist=10 * element_size, pml_param=alpha
)
# %%   # FEM computation
...
geo_param_fem["a_i"] = c * np.cosh(
    2 * np.arccosh(a / c) - np.arccosh(geo_param_fem["a_o"] / c)
)
geo_param_fem["b_i"] = np.sqrt(geo_param_fem["a_i"] ** 2 - c**2)
geo_param_fem.update(
    {
        "R1_TR": 1e-15,  # R_TR/R radius of truncated region,
        "x_ofst_1": -1.40 * R_DtN_fem,
        "y_ofst_1": np.pi - R_DtN_fem,
...
gmshfile = Num_utils.build_mesh(
    geofile_fem, geo_param_fem, gmsh_param_fem, gdim=2
)
(eigval_fem, dof_fem) = Num_FEM.compute_eigenvalues_one_corner(
    gmshfile,
    a,
    b,
    [phi],
...
ax.plot(
    ...
    np.imag(eigval_fem),
    color="C2",
    label=f"FEM mixed coords, DoF={dof_fem}",
    linestyle="none",
    marker="o",
    fillstyle="none",
)
ax.plot(
    np.real(eigval_bem),
    np.imag(eigval_bem),
    label=f"BIE, $P=${qnumber}, DoF={dof_bem}",
    linestyle="none",
    marker="x",
    fillstyle="none",
    color="C1",
...
ax.set_title(
    f"Eigenvalues, major axis perturbation: "
...
f.savefig(
    os.path.join(DIR_ARTICLE_IMG, "Num-Ellipse-1-Corner-Eigenvalues-FEM-BEM")
)
...
for n in range(0, N_track):
    ...
    ax.plot(
        [x_c, a * np.cos(cor_jun1_theta)],
        [y_c, b * np.sin(cor_jun1_theta)],
        color="C0",
    )
    ax.plot(
        [x_c, a * np.cos(cor_jun2_theta)],
        [y_c, b * np.sin(cor_jun2_theta)],
        color="C0",
    )
    R = np.linalg.norm(
        np.array([x_c, y_c])
...
fig.suptitle(
    f"BIE eigenfunctions, major axis perturbation: "
...
fig.savefig(
    os.path.join(DIR_ARTICLE_IMG, f"Num-Ellipse-1-Corner-major-eigenfunctions")
)
