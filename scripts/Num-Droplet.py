...
eigvals_f = np.array(
    [
        0.18160343579523447 - 0.0005568987991445146 * 1j,
...
def compute_eigenvalues_BEM(opts_eigv, **kwargs_julia):
    return Num_BEM.compute_NP_eigenvalues_droplet_isogeometric(
        opts_eigv,
        qorder=qorder,
...
for alpha in alpha_l:
    (eigvals, dof, phi, _, _, _) = compute_eigenvalues_BEM(
        opts_eigv,
...
element_size = 1 / 90
(eigvals, dof, phi, eigvecs, sl_pot, R_pml_corrected) = compute_eigenvalues_BEM(
    opts_eigv,
    meshsize=element_size,
    qmaxdist=10 * element_size,
    pml_param=alpha,
