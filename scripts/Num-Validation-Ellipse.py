...
geo_param = {
    "a_m": a,
    "b_m": b,  # sign-changing interface
    "a_d": 2 * a,
    "b_d": 2 * a,  # DtN boundary
...
for N_m in [30, 100]:
    ...
    (eigval, dof) = compute_eigenvalues_fem(
        gmshfile, DtN_order=30, quad_order=2, lambda_target=[0.2, -0.2]
    )
...
for N_m in [20, 500]:
    ...
    (eigval, dof, _, _) = compute_eigenvalues_bem(
        gmshfile, opts_eigv, qmaxdist=10 * a / N_m
    )
...
for i, offset in enumerate([0.15, 0.1]):
    ax.plot(
        ...
        np.imag(eigval_bem_l[i]) + offset,
        label=f"BIE, DoF={dof_bem_l[i]}",
        linestyle="none",
        marker=marker_l[i],
    )
marker_l = ["+", "x"]
for i, offset in enumerate([-0.1, -0.15]):
    ax.plot(
        np.real(eigval_fem_l[i]),
        np.imag(eigval_fem_l[i]) + offset,
...
ax.set_ylabel(
    r"$\max_{1\leq{}j\leq{}J}\,|\lambda^h_j-\lambda_j^{\text{el}}|\,/\,|\lambda_j^{\text{el}}|$"
)
...
for N_gamma in N_gamma_l:
    ...
    gmshfile = Num_utils.build_mesh(geofile, geo_param, gmsh_param, gdim=1)
    (eigval, dof, _, _) = compute_eigenvalues_bem(
        gmshfile, opts_eigv, qmaxdist=10 * a / N_gamma
    )
    eigval_l.append(eigval)
    dof_l.append(dof)
# %% Plot error
...
ax.set_xlabel(f"DoF / {int(min(dof_l)):d}")
ax.set_ylabel(
    r"$\max_{1\leq{}j\leq{}J}\,|\lambda_j^h-\lambda_j^{\text{el}}|\,/\,|\lambda_j^{\text{el}}|$"
)
...
geo_param["N_m"] = 100
gmshfile = Num_utils.build_mesh(geofile, geo_param, gmsh_param, gdim=1)
(eigval, dof, eigvecs, sl_pot) = compute_eigenvalues_bem(
    gmshfile, opts_eigv, qmaxdist=10 * a / geo_param["N_m"]
)
...
ax.plot(
    np.real(eigval),
    np.imag(eigval),
    label=f"$\lambda^h$, $P$={qnumber}, DoF={dof}",
    linestyle="none",
    marker="+",
    color="C1",
)
...
for i, idx in enumerate(plot_mode_idx):
    ...
    if i == 0:
        ...
        ax.set_yticklabels([])
    ax.set_title(
        rf"$\lambda^h$={eigv_val:1.1e}$\simeq\lambda_{{{idx}}}^{{\text{{el}}}}$"
    )
    ax.set_xlim([-d_x, d_x])
    ax.set_aspect("equal")
