...
def compute_eigenvalues_bem(opts_eigv: Num_BEM.EigenvalueSolverOpts, **kwargs):
    ...
    return Num_BEM.compute_NP_eigenvalues_ellipse_with_corner_isogeometric(
        a,
        b,
        x_c,
        y_c,
...
for alpha in alpha_l:
    (eigvals, dof, _, _) = compute_eigenvalues_bem(
        opts_eigv,
...
f.suptitle(
    f"Eigenvalues, minor axis perturbation: "
...
# figsize = mpl.rcParams['figure.figsize']
f, axs = plt.subplots(
    ncols=len(corner_grading_regularity_l),
    sharey=True,
    layout="constrained",
    figsize=figsize,
)
for idx, corner_grading in enumerate(
    corner_grading_regularity_l
):  # for each corner law
    bem = bem_comp_l[idx]
    ...
    for i in range(N_track):  # for each tracked eigenvalue
        ...
        for j, dof in enumerate(bem.dof_l):
            ...
            # Singular exponent
        eta_s = PEP_ana.compute_radial_corner_exponent_most_singular(
            phi, PEP_ana.np_to_contrast(eigval_f_ref[i]), pml_parameter=alpha
        )
    ...
    ax.set_title(
        f"{corner_grading_legend_l[idx]}\n"
        + rf"Reference at {bem_ref.dof_l[0]:d}$\simeq${bem_ref.dof_l[0] / min(bem.dof_l):2.2g}$\,\times\,${min(bem.dof_l)} DoF"
    )
f.suptitle(
    rf"Isogeometric BIE error, minor axis perturbation: "
    + f"($a$,$b$,$\phi$)$=$({a},{b},{phi / np.pi:2g}$\pi$)."
    + "\n"
    + rf"$P=${qnumber}, $\arg(\alpha)=\pi/${np.pi / np.angle(alpha):.2g}."
)
f.savefig(
    os.path.join(
        DIR_ARTICLE_IMG, f"Num-Ellipse-1-Corner-Minor-convergence-isogeo"
    )
)
