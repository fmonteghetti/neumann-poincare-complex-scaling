...
def plot_modulus_and_argument(fun2plot, cmap_arg, name):
    ...
    fig, axs = plt.subplots(
        nrows=2, ncols=2, layout="constrained", figsize=figsize
    )
    # Zoomed-out plot
    ax = axs[0, 0]
    ...
    # y-value
    ax.plot(
        y[0], y[1], marker="o", color="C2", fillstyle="none", linestyle="none"
    )
    ax.text(
        2.5 * y[0],
        y[1],
    ...
    for s, sym in zip(markers_pos, markers_sym):
        marker_z = fun2plot(
            line_fun(s)[0], line_fun(s)[1], y[0], y[1], alpha, R_alpha=R_alpha
        )
    ...
    ax.grid()
    fig.suptitle(
        rf"Plot of $q_\alpha(\cdot,\boldsymbol{{y}})$ in scaling region $B_\alpha$. "
        + rf"$\alpha=\exp(i\pi/{{{np.pi / np.angle(alpha):g}}})$"
        + rf", $R_\alpha={{{R_alpha}}}$"
        + rf", $|\boldsymbol{{y}}|={{{np.linalg.norm(y) / R_alpha}}}R_\alpha$."
    )
...
for ss, sym in zip(markers_pos, markers_sym):
    marker_z = fun2plot(
        line_fun(ss)[0], line_fun(ss)[1], y[0], y[1], alpha, R_alpha=R_alpha
    )
...
z = fun2plot(s * y[0], s * y[1], y[0], y[1], alpha, R_alpha=R_alpha)
ax.plot(
    s, np.imag(z), label=r"$G_\alpha(\cdot,\boldsymbol{y})$", linestyle="dashed"
)
for s, sym in zip(markers_pos, markers_sym):
    marker_z = fun2plot(
        line_fun(s)[0], line_fun(s)[1], y[0], y[1], alpha, R_alpha=R_alpha
...
ax.grid()
fig.suptitle(
    rf"Comparison of $\mathfrak{{G}}_\alpha(\cdot,\boldsymbol{{y}})$ and $G_\alpha(\cdot,\boldsymbol{{y}})$ along the curve $\mathcal{{C}}$"
)
