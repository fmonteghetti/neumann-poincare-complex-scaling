...
def compute_eigenvalues_bem(gmshfile, opts_eigv, **kwargs_julia):
    return Num_BEM.compute_NP_eigenvalues_corner(
        gmshfile,
        opts_eigv,
        qorder=qorder,
...
for alpha in alpha_l:
    (eigval, dof) = compute_eigenvalues_bem(
        gmshfile, opts_eigv, qmaxdist=10 * geo_param["lc_d"], pml_param=alpha
    )
...
for i, alpha in enumerate(alpha_l):
    ...
    if i == 0:
        ...
        ax.add_artist(
            ax.legend(loc="lower left", ncols=1, handles=handles[-1:])
        )
    ax.grid(True)
...
for i, alpha in enumerate(alpha_l):
    ...
    if i == 0:
        ...
        ax.add_artist(ax.legend(loc="upper left", ncols=1, handles=handles[:2]))
        ax.add_artist(
            ax.legend(loc="lower left", ncols=1, handles=handles[-2:])
        )
    ax.grid(True)
