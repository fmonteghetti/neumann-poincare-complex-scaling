...
angle_l = np.array(
    [0, np.pi / 10, np.pi / 3.1, np.pi / 2.5]
)  # argument of scaling parameter alpha (rad)
...
fig, axs = plt.subplots(
    ncols=np.size(alpha_l), nrows=1, sharex=True, sharey=True, figsize=figsize
)
for i, alpha in enumerate(alpha_l):
    ...
    ax.fill(
        np.real(bnd2), np.imag(bnd2), color="C1", alpha=0.4, label=r"$U_\alpha$"
    )
    s = ess_spectrum(eta, phi, alpha=alpha)
    ax.plot(
        np.real(s), np.imag(s), label=r"$\sigma_{\text{ess}}(K^\star_\alpha)$"
    )
...
if phi <= np.pi:
    ...
    ax.set_xticklabels(
        [r"$\kappa_{\phi}$", r"$-1$", r"$1/\kappa_\phi$", r"$0$"]
    )
else:
    ax.set_xticks([1 / kappa_phi, -1, kappa_phi, 0])
    ax.set_xticklabels(
        [r"$1/\kappa_{\phi}$", r"$-1$", r"$\kappa_\phi$", r"$0$"]
    )
ax.grid()
