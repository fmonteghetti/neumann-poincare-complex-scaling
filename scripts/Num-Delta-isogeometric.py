...
eigvals_f = np.array(
    [
        0.19737032258876291 + 1.3292996359276421e-5 * 1j,
...
def compute_eigenvalues_BEM(opts_eigv, **kwargs_julia):
    return Num_BEM.compute_NP_eigenvalues_delta_isogeometric(
        phi,
        opts_eigv,
        qorder=qorder,
...
for alpha in alpha_l:
    (eigvals, dof, _, _, _) = compute_eigenvalues_BEM(
        opts_eigv,
    ...
    if i != 0:
        ...
        for k, l in np.ndenumerate(np.sort(eigvals_f[np.real(eigvals_f) < 0])):
            ...
            ax.annotate(
                ...
                xy=(np.real(l), np.imag(l)),
                xytext=(
                    np.cos(theta) * annotation_arrow_length,
                    np.sin(theta) * annotation_arrow_length,
                ),
                textcoords="offset points",
                arrowprops=dict(arrowstyle="->", color="C1"),
                color="C1",
...
f.suptitle(
    f"BIE eigenfunctions, delta geometry: $\phi={{{phi_frac.numerator}}}\pi/{{{phi_frac.denominator}}}$."
)
...
element_size = 1 / 20
(eigvals, dof, eigvecs, sl_pot, R_pml_corrected) = compute_eigenvalues_BEM(
    opts_eigv,
    meshsize=element_size,
    qmaxdist=10 * element_size,
    pml_param=alpha,
