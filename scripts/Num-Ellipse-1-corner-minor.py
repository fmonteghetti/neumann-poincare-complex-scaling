...
for alpha in alpha_l:
    (eigval,dof,_,_) = compute_eigenvalues_BEM(gmshfile, opts_eigv,
                            qmaxdist = qmaxdist(element_size), pml_param=alpha)
...
f.suptitle(f"Eigenvalues, minor axis perturbation: $(a,b,\phi)=$"
           +fr"({a},{b},{phi/np.pi:.2g}$\pi$).")
f.savefig(os.path.join(DIR_ARTICLE_IMG,'Num-Ellipse-1-Corner-Minor-Eigenvalues'))
...
geo_param['cor_lc'] = Num_utils.corner_mesh_grading_power(element_size_ref,
                                                            power=grading_power) 
gmshfile = Num_utils.build_mesh(geofile,geo_param,gmsh_param,gdim=1)
(eigval_ref,dof_ref,_,_) = compute_eigenvalues_BEM(gmshfile, opts_eigv,
                        qmaxdist = qmaxdist(element_size_ref), pml_param=alpha) 
...
# plt.style.use(MPLSTYLE_VSCODE)
# figsize = mpl.rcParams['figure.figsize']
f, axs = plt.subplots(ncols=len(alpha_l),sharey=True,
                      layout='constrained',figsize=figsize)
for (k,alpha) in enumerate(alpha_l):
    ...
    if np.imag(alpha)>0:
        ax.set_title(fr'$\arg(\alpha)=\pi/${np.pi/np.angle(alpha):.2g}')
    else:
        ax.set_title(fr'$\arg(\alpha)=0$')
...
f.suptitle(f'BEM error, minor axis perturbation: '
           ...
           +fr'reference: {dof_ref:d}$\simeq${dof_ref/min(dof_l):2.3g}$\,\times\,${min(dof_l)} DoF.')
f.savefig(os.path.join(DIR_ARTICLE_IMG,f'Num-Ellipse-1-Corner-Minor-convergence-mesh'))
...
fig.savefig(os.path.join(DIR_ARTICLE_IMG,f'Num-Ellipse-1-Corner-minor-eigenfunctions'))