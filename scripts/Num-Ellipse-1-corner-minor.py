...
This script uses a Nyström discretization of the NP eigenvalue problem.
...
from np_complex_scaling.Num_utils import (
    DIR_ARTICLE_IMG,
    MPLSTYLE_ARTICLE,
    MPLSTYLE_VSCODE,
)
...
opts_eigv = Num_BEM.EigenvalueSolverOpts(
    use_Arnoldi=False, tol=1e-8, nev=10, shift=[2e-1, -2e-1, -5e-2, 5e-2]
)
grading_power = 4  # agressive refinment
# Geometry
(a, b, phi) = (2.5, 1, np.pi * 0.85)
corner_pos = "top"
(x_c, y_c, x_m, y_m, R) = PEP_ana.get_C1corner_ellipse(
    a, b, phi, pos=corner_pos
)
...
def compute_eigenvalues_BEM(gmshfile, opts_eigv, **kwargs_julia):
    return Num_BEM.compute_NP_eigenvalues(
        gmshfile,
        opts_eigv,
        qorder=qorder,
...
element_size = a / 70
qmaxdist = lambda element_size: 10 * element_size
opts_eigv.use_Arnoldi = False
geo_param["lc_m"] = element_size
geo_param["cor_lc"] = Num_utils.corner_mesh_grading_power(
    element_size, power=grading_power
)
...
for alpha in alpha_l:
    (eigval, dof, _, _) = compute_eigenvalues_BEM(
        gmshfile, opts_eigv, qmaxdist=qmaxdist(element_size), pml_param=alpha
    )
...
for i, alpha in enumerate(alpha_l):
    ax = axs.ravel()[i]
    eigval_ex = Num_utils.compute_exact_spectrum_corner(
        phi, pml_parameter=alpha
    )
    ax.plot(
        np.real(eigval_ex),
        np.imag(eigval_ex),
    ...
    ax.plot(
        [np.abs(np.pi - phi) / (2 * np.pi), -np.abs(np.pi - phi) / (2 * np.pi)],
        [0, 0],
        linestyle="none",
        label=r"$\pm\,\lambda_c(\phi)$",
        marker="o",
        color="C0",
    )
    eigval_ex = Num_utils.compute_exact_eigenvalues_ellipse(
        np.min([b / a, a / b]), N=20
    )
    ax.plot(
        np.real(eigval_ex),
        np.imag(eigval_ex),
        label=r"$\lambda^{\text{el}}$",
        marker="s",
        fillstyle="none",
        linestyle="none",
        color="C0",
    )
    eigval_f = Num_utils.filter_eigenvalues_ellipse(
        eigval_bem_l[i], np.min([b / a, a / b]), N=4
    )
    if i != 0:
        for k, l in np.ndenumerate(eigval_f[np.real(eigval_f) > 0]):
            ...
            ax.annotate(
                ...
                xytext=(0, sign * annotation_arrow_length),
    ...
    ax.plot(
        np.real(eigval_bem_l[i]),
        np.imag(eigval_bem_l[i]),
...
f.suptitle(
    f"Eigenvalues, minor axis perturbation: $(a,b,\phi)=$"
    + rf"({a},{b},{phi / np.pi:.2g}$\pi$)."
)
f.savefig(
    os.path.join(DIR_ARTICLE_IMG, "Num-Ellipse-1-Corner-Minor-Eigenvalues")
)
# %% Plot self-convergence of eigenvalues
element_size_l = [a / (2**N) for N in np.linspace(0, 5, num=7)]
element_size_ref = a / (2**7)
qmaxdist = lambda element_size: R / 2
...
bem_l: typing.List[SpectralComputation] = []
for alpha in alpha_l:
    ...
    for i, element_size in enumerate(element_size_l):
        ...
        (eigval, dof, _, _) = compute_eigenvalues_BEM(
            gmshfile,
            opts_eigv,
            qmaxdist=qmaxdist(element_size),
            pml_param=alpha,
        )
        eigval_l.append(eigval)
        dof_l.append(dof)
    bem_l.append(SpectralComputation(eigval_l, dof_l, qnumber))
# Compute reference value
geo_param["lc_m"] = element_size_ref
geo_param["cor_lc"] = Num_utils.corner_mesh_grading_power(
    element_size_ref, power=grading_power
)
gmshfile = Num_utils.build_mesh(geofile, geo_param, gmsh_param, gdim=1)
(eigval_ref, dof_ref, _, _) = compute_eigenvalues_BEM(
    gmshfile, opts_eigv, qmaxdist=qmaxdist(element_size_ref), pml_param=alpha
)
bem_ref = SpectralComputation([eigval_ref], [dof_ref], qnumber)
# Plot
N_track = 4  # track the first N_track eigenvalues
order_npts = len(element_size_l)
plt.style.use(MPLSTYLE_ARTICLE)
[paperwidth, paperheight] = mpl.rcParams["figure.figsize"]
figsize = (0.63 * paperwidth, 0.25 * paperheight)
# plt.style.use(MPLSTYLE_VSCODE)
# figsize = mpl.rcParams['figure.figsize']
f, axs = plt.subplots(
    ncols=len(alpha_l), sharey=True, layout="constrained", figsize=figsize
)
for k, alpha in enumerate(alpha_l):
    ...
    for i in range(N_track):  # for each tracked eigenvalue
        error = list()
        for j, dof in enumerate(dof_l):
            eigval_f = Num_utils.filter_eigenvalues_ellipse(
                bem.eigval_l[j], aspect_ratio=np.min([b / a, a / b]), N=N_track
            )
            eigval_f_ref = Num_utils.filter_eigenvalues_ellipse(
                bem_ref.eigval_l[0],
                aspect_ratio=np.min([b / a, a / b]),
                N=N_track,
            )
            # Maximum error on i-th eigenvalue
            error.append(
                np.abs(eigval_f[i] - eigval_f_ref[i]) / np.abs(eigval_f_ref[i])
            )
        p = Num_utils.compute_slope(np.array(bem.dof_l), error, N=order_npts)
        if (i <= 1) or (np.imag(alpha) > 0):
            ax.plot(
                np.array(bem.dof_l) / min(bem.dof_l),
                error,
                fillstyle="none",
    ...
    if k == 0:
        ax.set_ylabel(
            r"$|\lambda^h-\lambda^{\text{ref}}|\,/\,|\lambda^{\text{ref}}|$"
        )
    if np.imag(alpha) > 0:
        ax.set_title(rf"$\arg(\alpha)=\pi/${np.pi / np.angle(alpha):.2g}")
    else:
        ax.set_title(rf"$\arg(\alpha)=0$")
...
f.suptitle(
    f"BIE error, minor axis perturbation: "
    + f"($a$,$b$,$\phi$)=({a},{b},{phi / np.pi:2g}$\pi$)."
    + "\n"
    + f"$P$={qnumber}, "
    + f"grading: $h_c=h^{{{grading_power}}}$, "
    + rf"reference: {dof_ref:d}$\simeq${dof_ref / min(dof_l):2.3g}$\,\times\,${min(dof_l)} DoF."
)
f.savefig(
    os.path.join(
        DIR_ARTICLE_IMG, f"Num-Ellipse-1-Corner-Minor-convergence-mesh"
    )
)
...
gmshfile = Num_utils.build_mesh(geofile, geo_param, gmsh_param, gdim=1)
(eigvals, dof, eigvecs, sl_pot) = compute_eigenvalues_BEM(
    gmshfile, opts_eigv, qmaxdist=qmaxdist(element_size), pml_param=alpha
)
eigvals_f = Num_utils.filter_eigenvalues_ellipse(
    eigvals, aspect_ratio=np.min([b / a, a / b]), N=N_track
)
plt.style.use(MPLSTYLE_ARTICLE)
[paperwidth, paperheight] = mpl.rcParams["figure.figsize"]
...
for n, ax in enumerate(fig.axes):
    ...
    spanx = np.linspace(-1.10 * a, 1.10 * a, num=int(2e2))
    spany = np.linspace(-a, a, num=int(2e2))
    ...
    R = np.linalg.norm(
        np.array([x_c, y_c])
        - np.array([a * np.cos(cor_jun1_theta), b * np.sin(cor_jun1_theta)])
    )
    ...
    if n % 2 == 0:
        ax.set_ylabel("$y$")
    ax.set_title(
        f"$\lambda^h_{n + 1}\simeq${eigvals[eigv_idx]:1.1e}"
        + rf"$\simeq\lambda^{{\text{{el}}}}_{{{n + 1}}}$"
    )
...
fig.suptitle(
    f"BIE eigenfunctions, minor axis perturbation: "
    + f"($a$,$b$,$\phi={{{phi / np.pi:.2g}}}\pi$)."
    + "\n"
    + f"$P=${qnumber}, DoF$=${dof}, "
    + rf"$\arg(\alpha)=\pi/{{{np.pi / np.angle(alpha):.2g}}}$."
)
fig.savefig(
    os.path.join(DIR_ARTICLE_IMG, f"Num-Ellipse-1-Corner-minor-eigenfunctions")
)
