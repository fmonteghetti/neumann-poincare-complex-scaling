...
def ellipse_parameters(case_name):
    ...
    if case_name == "JCP2021_figure_9":
        ...
        a_m = 2.5
        b_m = 1
        c = np.sqrt(a_m**2 - b_m**2)  # arbitrary
...
def perturbed_ellipse_parameters(case_name):
    ...
    if case_name == "JCP2021_case_A":
        ...
        phi = [0.75 * np.pi]
        corner_pos = ["left"]
        (x_c, y_c, x_m, y_m, R) = get_C1corner_ellipse(
            a_m, b_m, phi[0], pos=corner_pos[0]
        )
        a_d = np.abs(x_c) + 1.5 * R
        b_d = np.sqrt(a_d**2 - c**2)
        x_c = [x_c]
        y_c = [y_c]
        x_m = [x_m]
...
def compute_radial_corner_exponents(phi, kappa, tol=1e-10):
    ...
    add_trans = lambda eta, N: np.reshape(
        eta + 1j * 2 * q * np.c_[-N : N + 1], -1
    )
    N = 4 * q
    ...
    eta_even, eta_odd = (
        np.hstack((eta_even, eta_im)),
        np.hstack((eta_odd, eta_im)),
    )
...
def compute_radial_corner_exponents_alt(phi, kappa, tol=1e-10):
    ...
    def f_even_grad(eta, l, phi):
        return 2 * l * np.pi * np.cosh(eta * np.pi) + (np.pi - phi) * np.cosh(
            eta * (np.pi - phi)
        )
    ...
    x = np.linspace(
        0, max_real_part, num=np.int(max_real_part / step_real_part)
    )
    y = np.linspace(
        -max_imag_part,
        max_imag_part,
    ...
    (eta_even, converged, zeroder) = newton(
        lambda x: f_even(x, l, phi),
        x0=eta0.flatten(),
        fprime=lambda x: f_even_grad(x, l, phi),
        tol=tol,
        rtol=tol,
        full_output=True,
    )
    ...
    (eta_odd, converged, zeroder) = newton(
        lambda x: f_odd(x, l, phi),
        x0=eta0.flatten(),
        fprime=lambda x: f_odd_grad(x, l, phi),
        tol=tol,
        rtol=tol,
        full_output=True,
    )
