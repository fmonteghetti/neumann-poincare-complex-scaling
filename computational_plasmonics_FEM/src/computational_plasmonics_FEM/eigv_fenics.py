...
def assemble_PEP_no_PML(
    ...
    A_m = fe.PETScMatrix()
    A_d = fe.PETScMatrix()
    fenics_utils.assemble_GEP(
        A_m, a_m, A_d, a_d, bcs, diag_A=diag_Am, diag_B=diag_Ad
    )
    return A_m.mat(), A_d.mat()


def assemble_PEP_no_PML_mixed(
    W,
    dmesh,
    tags_omegaM,
    tags_omegaD,
    ...
    for i in range(len(tags_omegaD)):
        ...
        # Assemble sparse eigenvalue problem
    A_lhs = fe.PETScMatrix()
    A_rhs = fe.PETScMatrix()
    fenics_utils.assemble_GEP(
        A_lhs,
        sum(a_lhs),
        A_rhs,
...
def assemble_PEP_no_PML_DG_BZ(
    ...
    if eta != 0:
        a_lhs.append(
            spen(h_avg) * eta / h_avg * dot(jump(u, n), jump(v, n)) * dS
        )
    for tag in tags_Dirichlet:
...
def assemble_PEP_no_PML_DG_BZM(
    p,
    dmesh,
    tags_omegaM,
    tags_omegaD,
...
def assemble_PEP_no_PML_DG_LDG(
    p,
    dmesh,
    tags_omegaM,
    tags_omegaD,
    ...
    if beta[0] != 0 or beta[1] != 0:
        a_rhs.append(
            dot(jump(u, n), beta * jump(psi, n)) * dS
            - dot(beta * jump(phi, n), jump(v, n)) * dS
        )
    for tag in tags_Dirichlet:
        a_rhs.append(
            dot(u * n, psi) * ds(tag)
...
def assemble_PEP_PML(
    ...
    if per.N == 0:
        ...
        V = fe.FunctionSpace(
            dmesh.mesh, fe.MixedElement((FE, FE)), constrained_domain=per
        )
    Vr = V.sub(0)
    Vi = V.sub(1)
    # Dirichlet boundary condition
    ...
    # x (imag(1/alpha))
    a_m[4] = -a_theta(ui, vr, tags_omegaM_PML) + a_theta(
        ur, vi, tags_omegaM_PML
    )
    a_d[4] = -a_theta(ui, vr, tags_omegaD_PML) + a_theta(
    ...
    for i in range(len(A_m)):
        A_m[i] = fe.PETScMatrix()
        A_d[i] = fe.PETScMatrix()
        fenics_utils.assemble_GEP(
            A_m[i],
            a_m[i],
            A_d[i],
...
def get_pbc_ellipse1corner(a_m, b_m, phi, R_TR, x_offset, y_offset, tol=1e-5):
    ...
    def EulerBot_map(
        x,
    ):  # Bottom boundary {Log(R_TR) < x-x_offset < Log(R), y-y_offset=-pi}
        return (
            fe.near(x[1], y_offset - fe.DOLFIN_PI)
            and ((x[0] - x_offset - np.log(R)) < tol)
            and ((x[0] - x_offset - np.log(R_TR)) > -tol)
        )

    def EulerTop_map(
        x,
    ):  # Top boundary {Log(R_TR) < x-x_offset < Log(R), y-y_offset=pi}
        return (
            fe.near(x[1], y_offset + fe.DOLFIN_PI)
            and ((x[0] - x_offset - np.log(R)) < tol)
            and ((x[0] - x_offset - np.log(R_TR)) > -tol)
    ...
    def EulerRight_map(
        x,
    ):  # rectangle right {x-x_offset=log(R), -pi<y-y_offset<pi}
        return (
            fe.near(x[0], np.log(R) + x_offset)
            and ((x[1] - y_offset - np.pi) < tol)
            and ((x[1] - y_offset + np.pi) > -tol)
        )

    def map_Circle_to_EulerRight(x, y):
        y[0] = (
            0.5 * np.log((x[0] - x_c) ** 2 + x[1] ** 2) + x_offset
        )  # ln(r(x,y))+x_offset
