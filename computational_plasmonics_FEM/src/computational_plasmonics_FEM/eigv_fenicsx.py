...
def assemble_PEP_PML_fenicsx(
    ...
    tags: dict(list(int))
        ...
        'gamma-d': Dirichlet(to-Neumann) boundary.
    ...
    DtN_order: int, optional
        Order of the Dirichlet-to-Neumann boundary condition to apply on the
        boundary gamma-d. If 0, a Dirichlet boundary condition is applied.
    ...
    if DtN_order == 0:
        bcs = fenicsx_utils.create_DirichletBC(
            dmesh.mesh, dmesh.facet_tags, V, [uD], tags_Dirichlet
        )
    ...
    if (len(tags_omegaM_PML) > 0) and (len(tags_omegaD_PML) > 0):
        if split_real_imag:
            # x (real(alpha))
            a_m["real-a"] = a_z(ur, vr, tags_omegaM_PML) + a_z(
                ui, vi, tags_omegaM_PML
            )
            a_d["real-a"] = a_z(ur, vr, tags_omegaD_PML) + a_z(
                ui, vi, tags_omegaD_PML
            )
            # x (imag(alpha))
            a_m["imag-a"] = -a_z(ui, vr, tags_omegaM_PML) + a_z(
                ur, vi, tags_omegaM_PML
            )
            a_d["imag-a"] = -a_z(ui, vr, tags_omegaD_PML) + a_z(
                ur, vi, tags_omegaD_PML
            )
            # x (real(1/alpha))
            a_m["real-ainv"] = a_theta(ur, vr, tags_omegaM_PML) + a_theta(
                ui, vi, tags_omegaM_PML
            )
            a_d["real-ainv"] = a_theta(ur, vr, tags_omegaD_PML) + a_theta(
                ui, vi, tags_omegaD_PML
            )
            # x (imag(1/alpha))
            a_m["imag-ainv"] = -a_theta(ui, vr, tags_omegaM_PML) + a_theta(
                ur, vi, tags_omegaM_PML
            )
            a_d["imag-ainv"] = -a_theta(ui, vr, tags_omegaD_PML) + a_theta(
                ur, vi, tags_omegaD_PML
            )
        else:
            # x (alpha)
    ...
    for key in a_m:
        ...
        A_m[key] = dolfinx_mpc.assemble_matrix(
            dolfinx.fem.form(a_m[key]), mpc, bcs=bcs, diagval=diag_Am
        )
        print(f"Assembly a_d ({key})")
        A_d[key] = dolfinx_mpc.assemble_matrix(
            dolfinx.fem.form(a_d[key]), mpc, bcs=bcs, diagval=diag_Ad
        )
    if DtN_order != 0 and split_real_imag:
        raise ValueError(
            "DtN boundary condition not implement with split Re/Im."
        )
    if DtN_order != 0 and (not split_real_imag):  # add DtN contribution to A_d
        k_fun = dolfinx.fem.Function(V)
        A_tmp = None
        print(f"Assembly of DtN boundary condition")
        for n in np.arange(1, DtN_order + 1):
            for m in [0, 1]:
                (alpha, k_expr) = PEP_utils_ana.DtN_Laplace_circle(n, m)
                k_fun.interpolate(k_expr)
                A_tmp = fenicsx_utils.assemble_matrix_double_facet_integral(
                    k_fun, tags_Dirichlet, dmesh, result=A_tmp
                )
                A_d["1"].axpy(-alpha, A_tmp)
    # -- Indices of real and imaginary parts
...
class PEP_with_PML_fenicsx:
    ...
    def __init__(
        ...
        # Order of Dirichlet to Neumann boundary condition (0 for Dirichlet)
        self.DtN_order = DtN_order
        ...
        self.idx_ui = (
            None  # indices of imaginary part (if split_real_imag==True)
        )
        self.V_r = None  # subspace for real part (if split_real_imag==True)
        self.V_i = (
            None  # subspace for imaginary part (if split_real_imag==True)
        )
        # Eigenvalue problem
    ...
    def assemble(self, V_fem):
        ...
        if self.split_real_imag:
            self.idx_ur = out[2]
            self.idx_ui = out[3]
            self.V_r = out[4].collapse()[0]
            self.V_i = out[5].collapse()[0]
        print("Done.")

    def solve(self, SLEPc_params, alpha=1.0, tolReIm=1e-5):
        self.alpha = alpha
        A_m_l, A_d_l = self.A_m_l, self.A_d_l
        coeff = dict()
        coeff = {
            "1": 1.0,
            "a": alpha,
            "ainv": 1 / alpha,
            "real-a": np.real(alpha),
            "imag-a": np.imag(alpha),
            "real-ainv": np.real(1 / alpha),
            "imag-ainv": np.imag(1 / alpha),
        }
        A_m, A_d = 0, 0

        if (
            ("a" in A_m_l.keys())
            and (isinstance(alpha, complex))
            and (PETSc.ComplexType != PETSc.ScalarType)
        ):
            raise ValueError(
                "PETSc.ScalarType is real. Set split_real_imag=True."
            )
        ...
        if self.split_real_imag:
            ...
            self.eigvec_i = list()
            for vec in eigvec:
                self.eigvec_r.append(vec[idx_ur])
                self.eigvec_i.append(vec[idx_ui])
        else:
    ...
    def set_deformed_critical_interval(self, phi_list):
        ...
        for phi in phi_list:
            psi = lambda eta: (
                np.sinh(eta * np.pi) / np.sinh(eta * (np.pi - phi))
            )
            self.k_Ic_even.append(
                lambda eta, alpha: (
                    (1 - psi(eta * alpha)) / (1 + psi(eta * alpha))
                )
            )
            self.k_Ic_odd.append(
                lambda eta, alpha: (
                    (1 + psi(eta * alpha)) / (1 - psi(eta * alpha))
                )
            )
    ...
    def plot_spectrum(self, f, eta=0, N=10, plot_mesh_name=False):
        ...
        if self.k_Ic_even is not None:
            for i in range(len(self.k_Ic_even)):
                if (
                    len(self.k_Ic_even) == 1
                ):  # do not number interval if there is only one corner
                    label = r"$I_{c,\alpha}^{\rm{even}}$"
                else:
                    label = r"$I_{c,\alpha}^{\rm{even}}$" + f"(#{i + 1})"
                ax.plot(
                    np.real(self.k_Ic_even[i](eta, alpha)),
                    np.imag(self.k_Ic_even[i](eta, alpha)),
                    label=label,
                    linestyle=Ic_linestyle[np.mod(i, len(Ic_linestyle))],
                    marker="None",
                    mfc="none",
                    color="b",
                )
        if self.k_Ic_odd is not None:
            for i in range(len(self.k_Ic_odd)):
                ...
                ax.plot(
                    np.real(self.k_Ic_odd[i](eta, alpha)),
                    np.imag(self.k_Ic_odd[i](eta, alpha)),
                    label=label,
                    linestyle=Ic_linestyle[np.mod(i, len(Ic_linestyle))],
                    marker="None",
                    mfc="none",
        ...
        if (self.k_even is not None) and (len(self.k_even(N)) != 0):
            ax.plot(
                np.real(self.k_even(N)),
                np.imag(self.k_even(N)),
                label=r"$\kappa_{\rm{exact}}^{\rm{even}}$",
                linestyle="none",
                marker="o",
                mfc="none",
                color="b",
        ...
        if (self.k_odd is not None) and (len(self.k_odd(N)) != 0):
            ax.plot(
                np.real(self.k_odd(N)),
                np.imag(self.k_odd(N)),
                label=r"$\kappa_{\rm{exact}}^{\rm{odd}}$",
                linestyle="none",
                marker="o",
                mfc="none",
                color="r",
        ...
        if (self.k_unp_even is not None) and (len(self.k_unp_even(N)) != 0):
            ax.plot(
                np.real(self.k_unp_even(N)),
                np.imag(self.k_unp_even(N)),
                label=r"$\kappa_{\rm{unp}}^{\rm{even}}$",
                linestyle="none",
                marker="s",
                mfc="none",
                color="b",
        ...
        if (self.k_unp_odd is not None) and (len(self.k_unp_odd(N)) != 0):
            ax.plot(
                np.real(self.k_unp_odd(N)),
                np.imag(self.k_unp_odd(N)),
                label=r"$\kappa_{\rm{unp}}^{\rm{odd}}$",
                linestyle="none",
                marker="s",
                mfc="none",
                color="r",
        ...
        ax.plot(
            -np.real(self.eigval),
            -np.imag(self.eigval),
            label=r"FEM (" + f"{self.eigvec_r[0].size:d} DoF)",
            linestyle="none",
            marker="x",
            mfc="none",
            color="k",
        ...
        title = (
            r"Spectrum with $\arg(\alpha)=$"
            + f"{np.rad2deg(np.angle(alpha)):3.4g}°"
            + f"\n{self.case_name}"
        )
        if plot_mesh_name:
    ...
    def plot_eigenfunction_pyvista(self, ip):
        ...
        try:  # Approach w/o interpolation (fast but works for isoparametric element only)
            ...
            U_r, U_i, U_n = (
                U_r / np.max(np.abs(U_n)),
                U_i / np.max(np.abs(U_n)),
                U_n / np.max(np.abs(U_n)),
            )
            grid = fenicsx_utils.create_pyvista_UnstructuredGrid_from_mesh(
                self.dmesh.mesh
            )
            grid.clear_data()
            grid["ur"] = U_r
            ...
            warnings.warn(
                "No-interpolation approach failed. Trying interpolation..."
            )
            grid = None
    ...
    def plot_eigenfunction_pyvista_matplotlib(self, fig, ip):
        ...
        x_max = np.max(self.dmesh.mesh.geometry.x, 0)
        x_min = np.min(self.dmesh.mesh.geometry.x, 0)
        ...
        ax.set_title(
            f"$ Re(u)/|u|_inf, \kappa$={-self.eigval[i_plot]:4.4g} DoF={self.dmesh.mesh.geometry.x.shape[0]}"
        )
        ax.set_xlabel("$x$")
        ax.set_ylabel("$y$")
        ax.grid(False)
    ...
    def export_eigenfunction_pyvista(self, fname):
        ...
        for i in range(len(self.eigval)):
            ...
            if try_interpolation:  # Interpolation to mesh coordinates
                ...
                u_topology, u_cell_types, u_geometry = (
                    dolfinx.plot.create_vtk_mesh(self.V_r)
                )
                grid = pv.UnstructuredGrid(u_topology, u_cell_types, u_geometry)
...
def get_pbc_ellipseNcorners_fenicsx(
    a_m,
    b_m,
    phi,
    pos,
    ...
    for i, tags in enumerate(tag_EulerBot_bnd):
        pbc.add_topological_condition(
            ...
            master_map=lambda x, y_offset=y_offset[i]: EulerTop_map(
                x, y_offset
            ),
        )
    ...
    def map_EulerRight_to_LeftCircle(x, x_c, y_c, R, x_offset, y_offset):
        ...
        y[0] = x_c + R * np.cos(x[1] - y_offset)
        y[1] = y_c + R * np.sin(x[1] - y_offset)
        y[2] = x[2]
        return y
    ...
    def map_EulerRight_to_TopCircle(x, x_c, y_c, R, x_offset, y_offset):
        y = np.zeros(x.shape)
        # print(f"Radius on corner right boundary: R={np.exp(x[0]-x_offset)}")
        y[0] = x_c + R * np.sin(x[1] - y_offset)
        y[1] = y_c - R * np.cos(x[1] - y_offset)
        y[2] = x[2]
        return y

    for i, tags in enumerate(tag_EulerRight_bnd):
        (x_c, y_c, x_m, y_m, R) = PEP_utils_ana.get_C1corner_ellipse(
            a_m, b_m, phi[i], pos=pos[i]
        )
        if pos[i] == "left":
            slave_to_master_map = (
                lambda x, x_c=x_c, y_c=y_c, R=R, x_offset=x_offset[i], y_offset=y_offset[i]: (
                    map_EulerRight_to_LeftCircle(
                        x, x_c, y_c, R, x_offset, y_offset
                    )
                )
            )
        elif pos[i] == "top":
