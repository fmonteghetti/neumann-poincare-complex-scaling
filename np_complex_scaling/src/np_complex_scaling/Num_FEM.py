...
def compute_eigenvalues_Cartesian(
    gmshfile, DtN_order=0, quad_order=2, lambda_target=[0.2]
):
    ...
    # Assemble PEP
    PEP = PEP_utils.PEP_with_PML_fenicsx(dmesh, phys_tags, DtN_order=DtN_order)
    V_fem = ("CG", quad_order)
    PEP.assemble(V_fem)
...
def compute_eigenvalues_one_corner(
    gmshfile,
    a,
    b,
    phi,
    ...
    pbc = PEP_utils.get_pbc_ellipseNcorners_fenicsx(
        a,
        b,
        phi,
        corner_pos,
    ...
    phys_tags = dict()
    phys_tags["omega-m"] = [
        k for tag in tag_names["omega_m"] for k in phys_tag_2D[tag]
    ]
    phys_tags["omega-d"] = [
        k for tag in tag_names["omega_d"] for k in phys_tag_2D[tag]
    ]
    phys_tags["gamma-d"] = [
        k for tag in tag_names["gamma_d"] for k in phys_tag_1D[tag]
    ]
    phys_tags["omega-m-pml"] = [
        k for tag in tag_names["omega_m_pml"] for k in phys_tag_2D[tag]
    ]
    phys_tags["omega-d-pml"] = [
        k for tag in tag_names["omega_d_pml"] for k in phys_tag_2D[tag]
    ]
    # Assemble PEP
    PEP = PEP_utils.PEP_with_PML_fenicsx(
        dmesh, phys_tags, pbc=pbc, DtN_order=DtN_order
    )
    V_fem = ("CG", quad_order)
    PEP.assemble(V_fem)
    # Compute eigenvalues in several runs
...
def compute_eigenvalues_corner_Cartesian(
    gmshfile,
    quad_order=2,
    pml_param=1,
    ...
    bcs = fenicsx_utils.create_DirichletBC(
        dmesh.mesh, dmesh.facet_tags, V, [uD], tags["gamma-d"]
    )
    ...
    S = ufl.as_matrix(
        (
            (x[0] * x[0] / norms, x[0] * x[1] / norms),
            (x[0] * x[1] / norms, x[1] * x[1] / norms),
        )
    )
    adjS = ufl.as_matrix(
        (
            (x[1] * x[1] / norms, -x[0] * x[1] / norms),
    ...
    A_d_l = dict()
    for key in a_m:
        A_m_l[key] = dolfinx.fem.petsc.assemble_matrix(
            dolfinx.fem.form(a_m[key]), bcs, diag_Am
        )
        A_m_l[key].assemble()
        A_d_l[key] = dolfinx.fem.petsc.assemble_matrix(
            dolfinx.fem.form(a_d[key]), bcs, diag_Ad
        )
        A_d_l[key].assemble()
    coeff = dict()
    ...
    for target in lambda_target:
        ...
        (eigval_tmp, eigvec_r_tmp, eigvec_i_tmp) = SLEPc_utils.EPS_get_spectrum(
            EPS
        )
