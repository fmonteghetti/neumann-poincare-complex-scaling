...
# Define path
# Image folder
DIR_ARTICLE_IMG = str(Path(__file__).parents[3])
# Matplotlib style
MPLSTYLE_ARTICLE = [
...
if "scicomp_utils_mesh.gmsh_utils" in sys.modules:
    ...
    def build_mesh(geofile, geo_param={}, gmsh_param={}, gdim=2):
        ...
        gmsh_utils.generate_mesh_cli(
            geofile, gmshfile, gdim, parameters=geo_param, **gmsh_param
        )
        return gmshfile
...
def compute_exact_spectrum_corner(
    angle, pml_parameter=1, eta=np.logspace(-2, 2, num=int(1e4))
):
...
def filter_distance(x, y, x_target, y_target, dist=1e-1):
    ...
    for i in range(x.shape[0]):
        for j in range(x.shape[1]):
            distance = np.sqrt(
                (x[i, j] - x_target) ** 2 + (y[i, j] - y_target) ** 2
            )
